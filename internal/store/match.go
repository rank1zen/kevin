...
func (db *MatchStore) RecordMatch(ctx context.Context, match internal.Match) error {
	...
	matchStore.CreateMatchInBatch(&batch, postgres.Match{
		ID:       match.ID,
		Date:     match.Date,
	...
	for _, p := range match.Participants {
		matchStore.CreateParticipantInBatch(&batch, postgres.Participant{
...
func (db *MatchStore) GetMatchDetail(ctx context.Context, id string) (_ internal.MatchDetail, err error) {
	...
	for i := range 10 {
		...
		if err != nil {
			...
			detail.Participants[i] = toParticipantDetail(participants[i], postgres.Summoner{PUUID: riot.PUUID(puuid)}, nil, nil, nil)
			continue
		}
...
func (db *MatchStore) GetMatchlist(ctx context.Context, puuid riot.PUUID, start, end time.Time) (matches []internal.SummonerMatch, err error) {
	matchStore := postgres.MatchStore{Tx: db.Pool}
	...
	var (
		matchList       = []postgres.Match{}
		participantList = []postgres.Participant{}
		rankBeforeList  = []*postgres.RankFull{}
		rankAfterList   = []*postgres.RankFull{}
	)
	...
	for _, opt := range listRankOptions {
		...
		if id != nil {
			...
			rankBeforeList = append(rankBeforeList, &postgres.RankFull{Status: status, Detail: detail})
		} else {
		...
		if id != nil {
			...
			rankAfterList = append(rankAfterList, &postgres.RankFull{Status: status, Detail: detail})
		} else {
...
func (db *MatchStore) GetChampions(ctx context.Context, puuid riot.PUUID, start, end time.Time) ([]internal.SummonerChampion, error) {
	matchStore := postgres.MatchStore{Tx: db.Pool}

	return matchStore.GetSummonerChampions(ctx, puuid, start, end)
}

func toSummonerMatch(match postgres.Match, participant postgres.Participant, rankBefore, rankAfter *postgres.RankFull) internal.SummonerMatch {
...
func toParticipantDetail(participant postgres.Participant, summoner postgres.Summoner, currentRank, rankBefore, rankAfter *postgres.RankFull) internal.ParticipantDetail {
	result := internal.ParticipantDetail{
		Participant: internal.Participant{
			...
			ChampionLevel:        participant.ChampionLevel,
			TeamPosition:         convertStringToTeamPosition(participant.TeamPosition),
			SummonerIDs:          participant.SummonerIDs,
			Runes:                convertListToRunePage(participant.Runes),
...
func toRankStatus(rank *postgres.RankFull) internal.RankStatus {
